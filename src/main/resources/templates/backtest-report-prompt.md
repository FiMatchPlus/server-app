...
**주의사항**: 데이터에서 '총 수익률: -0.32%' 형태로 표시된 것은 서버에서 포맷팅한 것이며, 실제 원본 데이터는 소수점 형태입니다. AI는 원본 데이터의 실제 수치를 기반으로 분석해야 합니다.
...
1. **BacktestMetrics (포트폴리오 성과 지표)**:
   - **total_return**: 소수점 비율값 (0.0032이면 0.32%를 의미)
   - **annualized_return**: 소수점 비율값 (0.0018이면 0.18%를 의미)
   - **volatility**: 소수점 비율값 (0.0015이면 0.15%를 의미)
   - **sharpe_ratio**: 무차원 배수값 (1.89)
   - **max_drawdown**: 소수점 비율값 (-0.0049이면 -0.49%를 의미)
   - **win_rate**: 소수점 비율값 (0.57이면 57%를 의미)
   - **profit_loss_ratio**: 무차원 배수값 (1.32)
...
**중요: 지표 단위 해석 가이드**:
* **반드시 주의**: 데이터에서 '총 수익률: -0.32%', '최대 낙폭: -0.49%' 같은 형태로 보이는 것은 이미 포맷팅된 것. 실제 값은 소수점 형태 (-0.0032, -0.0049)로 제공됨.
* **BacktestMetrics 지표들** (total_return, annualized_return, volatility, max_drawdown, win_rate): 소수점 값이 퍼센트 의미. -0.0032 = -0.32%, -0.0049 = -0.49%
* **무차원 지표들** (sharpe_ratio, profit_loss_ratio): 배수값 그대로 사용. -0.47은 -0.47 (퍼센트가 아님)
* **BenchmarkMetrics 지표들**: 이미 % 단위로 저장됨. 25.5 = 25.5%
* **지수값들**: 절대값으로 제공됨