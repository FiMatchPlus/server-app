...
1. **BacktestMetrics (포트폴리오 성과 지표)**:
   - **total_return**: % 단위로 제공됨 (예: -32.50 = -32.50%)
   - **annualized_return**: % 단위로 제공됨 (예: 12.34 = 12.34%)
   - **volatility**: % 단위로 제공됨 (예: 18.75 = 18.75%)
   - **sharpe_ratio**: 무차원 배수값 (예: 0.82)
   - **max_drawdown**: % 단위로 제공됨 (예: -15.20 = -15.20%)
   - **win_rate**: % 단위로 제공됨 (예: 65.50 = 65.50%)
   - **profit_loss_ratio**: 무차원 배수값 (예: 1.45)
...
```json
{
  "total_return": {
    "value": "-32.50",
    "interpretation": "총 수익률이 -32.50%라는 것은 백테스트 기간 동안 포트폴리오가 32.50%의 손실을 기록했음을 의미합니다."
  },
  "max_drawdown": {
    "value": "-15.20", 
    "interpretation": "최대 낙폭이 -15.20%라는 것은 포트폴리오가 겪은 가장 큰 하락이 15.20%였음을 의미합니다."
  }
}
```

**중요**: 지표들이 이미 백분위(%) 단위로 제공됩니다. 추가 변환 없이 그대로 해석하세요!
...
**중요: 지표 단위 정보**:
* **total_return (총 수익률)**과 **max_drawdown (최대 낙폭)**은 이미 백분위(%) 단위로 제공됩니다.
* **올바른 이해**: -32.50이 들어오면 -32.50%로 해석하세요! 이것이 32.50% 손실을 의미합니다!
