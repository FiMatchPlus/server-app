...
**입력 데이터 구조**:
1. **백테스트 기본 정보**: 백테스트명, 실행기간, 실행시간
2. **성과 지표 (BacktestMetrics)**: 총 수익률, 샤프 비율, 최대 낙폭, 승률, 손익비 등
3. **포트폴리오 보유 현황**: 현재 보유 종목 및 수량 정보
4. **일별 평가액 데이터 (DailyEquityData)**: 
   - 날짜별 포트폴리오 총액, 주식 평가액, 현금 잔고
   - **개별 종목별 일별 평가액 시계열 데이터** 포함
5. **거래 기록**: 매도/매수 실행 로그 및 손절/익절 사례
6. **벤치마크 비교 데이터**: 벤치마크 수익률 및 성과 비교
...
5. **diversification_effect** (각 종목을 개별종목으로 투자했을 때와 같이 투자했을 때 분산효과):
   - **individual_vs_portfolio**: 제공된 개별 종목 시계열 데이터를 활용하여 각 종목을 개별 투자했을 때 예상 성과와 포트폴리오 투자 시 실제 성과 비교
   - **diversification_benefits**: 개별 종목들의 수익률 변동성을 기반으로 위험 감소 효과, 수익 안정성 개선 정도, 종목 간 상관관계 분석을 통해 분산 효과를 구체적으로 수치화하여 설명
